...
// Functor para elevar ao quadrado um valor
struct Square : public thrust::unary_function<double, double>
{
    __host__ __device__
...
int main()
{
    ...
    thrust::transform(difference.begin(), difference.end(), thrust::constant_iterator<double>(average), differenceFromAverage.begin(), Difference());

    double squaredDifferenceSum = thrust::transform_reduce(differenceFromAverage.begin(), differenceFromAverage.end(), Square(), 0.0, thrust::plus<double>());

    double variance = squaredDifferenceSum / dataSize;

    std::cout << "Variance of the differences: " << variance << std::endl;
